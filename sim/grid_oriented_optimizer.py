"""Grid-only block scheduler — optimises DSO-relevant stress metrics."""
...
def _block_dso_proxy_score(
    ...
    """
    Lexicographic score aligned with DSO KPI drivers:
    lower daily peak MW, fewer overload slots, fewer high-stress slots, then load cost.
    """
    trial = current_total.copy()
    for slot in block:
        trial[slot] += mw_fleet
    stress = trial / zone_capacity_mw
    return (
        float(trial.max()),
        int((stress >= 1.0).sum()),
        int((stress >= stress_warning_threshold).sum()),
        _block_grid_cost(block, current_total, mw_fleet, grid_load_penalty, slot_duration_h),
    )
...
def optimize_ev_load_grid_oriented(
    ...
    *,
    ...
    stress_warning_threshold: float = 0.95,
) -> tuple[pd.Series, pd.Series, float]:
    """
    Grid-optimized scheduler: each app group picks the allowed consecutive block
    that best reduces DSO-relevant stress (peak, overload, high-stress minutes).
    """
    ...
    if zone_capacity_mw is None:
        zone_capacity_mw = float(np.max(current_total) * 1.05)
