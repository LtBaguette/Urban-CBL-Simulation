...
def load_app_kpi(cfg: SimConfig | None = None) -> pd.DataFrame:
    ...
    mc_path = cfg.output_dir / "method_comparison.csv"
    if mc_path.exists():
        df = pd.read_csv(mc_path)
        df = df[df["method_key"].isin(ORDER_STORY)].copy()
        df = df.set_index("method_key").loc[ORDER_STORY].reset_index()
        return df.rename(columns={"method_key": "scenario"})
...
def load_method_chart_monthly_savings(cfg: SimConfig | None = None) -> pd.DataFrame:
    """Per-EV monthly savings for methods shown on graph_method_comparison.png."""
    cfg = cfg or load_config()
    mc_path = cfg.output_dir / "method_comparison.csv"
    if not mc_path.exists():
        raise FileNotFoundError(
            f"Missing {mc_path}; run scripts/run_method_comparison.py first."
        )
    from sim.method_comparison import _chart_dataframe

    return _chart_dataframe(pd.read_csv(mc_path))[
        ["method_key", "display_name", "monthly_savings_per_ev_eur"]
...
def plot_all_savings(kpi_df: pd.DataFrame, out_path: Path, cfg: SimConfig) -> None:
        ...
        if cust_eur == 0 and dso_eur == 0:
            ...
            if customer >= 15:
                ax.text(
                    x[i],
                    total_top + 6,
                    f"Customer EUR {cust_eur / 1000:.0f}k",
                    ha="center",
                    va="bottom",
                    fontsize=8,
                    color=COLOR_CUSTOMER,
            ...
            net_y = total_top + (34 if customer >= 15 else 10)
            ax.text(
                x[i],
                net_y,
    ...
    y_bottom = min(dso_k.min() - 18, -50) if dso_k.min() < 0 else 0
...
def plot_customer_monthly_savings(
    ...
    """Per-EV monthly bill savings for all methods on graph_method_comparison.png."""
    from sim.method_comparison import _app_adoption_rate

    x = np.arange(len(monthly_df))
    if "display_name" in monthly_df.columns:
        x_labels = monthly_df["display_name"].tolist()
        per_ev = monthly_df["monthly_savings_per_ev_eur"]
    else:
        x_labels = [LABELS[s] for s in monthly_df["scenario"]]
        per_ev = monthly_df["monthly_savings_per_ev_eur"]
    ...
    ax.set_xlabel("Charging method")
    ax.set_title(
        f"What drivers save each month by charging method\n"
        f"(vs immediate plug-in, {cfg.fleet.n_evs:,} EV fleet, {adoption_pct}% app adoption)",
        fontweight="bold",
    )
    ax.set_xticks(x)
...
def generate_stakeholder_pack(cfg: SimConfig | None = None) -> list[Path]:
    ...
    monthly_df = load_method_chart_monthly_savings(cfg)
    paths = [
